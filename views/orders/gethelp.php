 ...
 ?> 
        <div id="page-wrapper" class="gray-bg dashbard-1">
            ...
            <div class="wrapper wrapper-content animated fadeIn">
                <div class="remaining_sum">
                    <p>还可提现的余额（元）<b class="fr"><?=$bonus->bonus_type_id ==1?$user->flow_money_rec-$money:$user->flow_money_manager-$money;?></b></p>
                    <p>支持最大提现金额（元）<b class="fr"><?=$bonus->money?></b></p>
                </div>
                <div class="pay_way">
                ...
                <div class="pay_way pay_mon">
                    ...
                    <input type="text" class="form-control input_mon" placeholder="输入金额,0~<?=$bonus->money?>" onChange= "CheckNumber(this)" required>
                    
                    <p class="help_reminder">
                        <label>
                            <input type="checkbox" class="i_konw" value="">
                        </label>
    ...
    <script>
        function CheckNumber(obj) 
        {
            ...
            if(Number(obj.value) > <?=$bonus->money?> ||  Number(obj.value) < 0){
                alert('请输入0~<?=$bonus->money?>元的金额'); 
                obj.select(); 
                obj.focus(); 
                return false; 
            }
        ...
        if(money % 100 !== 0){
            alert('请输入整百的金额');return false; 
        }
        if(money  > <?=$bonus->money?> ||  money < 0){
                alert('请输入0~<?=$bonus->money?>元的金额'); ; return false; 
        }
        var data = {
                money:money,
                type :'<?=$bonus->bonus_type_id?>',
                pay_type:pay_type
        };
        ...
        $('.confirm1').unbind().on('click',function(){
            ...
            $.get('<?=Url::toRoute("user/validation")?>',{pwd:pwd},function(msg){
                    if(msg.status ==1){
                       $('.confirm1').unbind();
                        $.post('<?=Url::toRoute("orders/gethelp")?>',data,function(msg){
                            alert(msg.message);
                            if(msg.status== 1 ){
